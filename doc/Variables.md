...
private:
  ...
  fuse_core::TimeStamp stamp_;
  fuse_core::UUID uuid_;

public:
  SMART_PTR_DEFINITIONS(Position2DStamped);

  Position2DStamped(const fuse_core::TimeStamp& stamp, const fuse_core::UUID& device_id) :
    data{},
  ...
  const fuse_core::TimeStamp& deviceId() const { return device_id_; }
...
ROS library, we will use a `fuse_core::TimeStamp` (which subclasses `rclcpp::Time`) to hold the timestamp.
